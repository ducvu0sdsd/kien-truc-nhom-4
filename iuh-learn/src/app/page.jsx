...
export default function Home() {
  ...
  const handleSignIn = () => {
    ...
    api({ sendToken: false, body: { username, password }, type: TypeHTTP.POST, path: username.includes("admin") ? '/sign-in-with-admin' : '/sign-in-with-student', port: ports.authServiceURL })
      .then(res => {
        globalThis.localStorage.setItem('accessToken', res.accessToken)
    ...
    <div className="flex items-center justify-center w-full h-screen">
      <div className="flex rounded-lg overflow-hidden gap-3 border-[#e1e1e1] border-[1px]">
        ...
        <div className="flex flex-col w-[350px] justify-center items-center gap-3 px-[30px]">
          ...
          <span className="text-center text-[13px]">Chào mừng trở lại với IUH Learn, hãy đăng nhập vì tương lai của bạn</span>
